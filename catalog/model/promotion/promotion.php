...
class ModelPromotionPromotion extends Model {
	...
	public function addGuest($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "promotion_guest SET name = '" . $this->db->escape($data['name']) . "', identity_card = '" . $this->db->escape($data['identity_card']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', age = '" . $this->db->escape($data['age']) . "', date_month = '" . $this->db->escape($data['date_month']) . "', date_day = '" . $this->db->escape($data['date_day']) . "', date_year = '" . $this->db->escape($data['date_year']) . "', date_time = '" . $this->db->escape($data['date_time']) . "', date_added = NOW()");
